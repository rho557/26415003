...
class SettingsManager(object):
    ...
    def activate(self, prefsDir=None, customSettings={}):
        ...
        prefsFile = os.path.join(self._prefsDir, 'user-settings.conf')

        # Use the GSettings backend for fresh setup, otherwise use the json
        # backend since it appears a settings file is present.
        if os.path.exists(prefsFile) or prefsDir is not None:
            self.backendName = 'json'
        else:
            self.backendName = 'gsettings'
